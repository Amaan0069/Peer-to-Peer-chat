...
// - If a single name, return the first letter.
// - If multiple names, return the first letter of the first two names.
...
const messagesEndReff = useRef(null);
  ...
  const isAtBottom = () => {
    const { scrollTop, clientHeight, scrollHeight } = messagesEndReff.current;
    return scrollHeight - scrollTop <= clientHeight + 10; // Adjust buffer if needed
  };
  useEffect(() => {
    if (messagesEndReff.current) {
      if (isAtBottom()) {
        scrollToBottom();
        setUnreadCount(0); // Reset unread count
      } else {
        setUnreadCount((prev) => prev + 1); // Increase count
      }
    }
  }, [messages]);
    
  // Keep a ref of the latest activeContact
    ...
    if (user.id) {
      axios
        ...
        .then((res) => {
          ...
          // Initialize lastReadTimestamps for new contacts
          setLastReadTimestamps((prev) => {
            const updated = { ...prev };
            Object.keys(fetchedMessages).forEach((contactId) => {
              if (!updated[contactId]) {
                updated[contactId] = new Date().toISOString();
              }
            });
  ...
  // Auto-trigger search as the user types
  // WHen a user types a word, the regex should start comparing it
  useEffect(() => {
    if (searchQuery.trim()) {
      axios
  ...
  // Update a contact's details and mark as permanent
  ...
  const handleReceiveMessage = useCallback(
    (message) => {
      ...
      // If the message is for the open chat, update its last read timestamp
      if (
        activeContactRef.current &&
        activeContactRef.current.id === contactId &&
      ...
      // Update contacts: increment unreadCount if not active, update lastMessage, and reorder list
      setContacts((prev) => {
  ...
  // Remove previous temporary contact when switching active contact
  useEffect(() => {
    if (
  ...
  // Merge contacts derived from messages and calculate unreadCount
  useEffect(() => {
    ...
    setContacts((prev) => {
      const newContacts = contactIds.map((id) => {
        ...
        const lastRead =
          lastReadTimestamps[id] || new Date().toISOString();
        const msgs = messages[id];
        const lastMsg = msgs[msgs.length - 1];
        const unreadCount = msgs.filter(
          (msg) =>
            msg.sender !== user.id &&
            new Date(msg.timestamp) > new Date(lastRead)
        ).length;
        return {
          id,
  ...
  // Update contact details for contacts with default values
  useEffect(() => {
  ...
  // Reset unread count and update lastReadTimestamps when a chat is opened
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Left Panel giving width and adjusting it*/}
      <div className="w-[30%] flex flex-col bg-white border-gray-400 relative border-r ">
        {/* Header */}
        ...
        {searchResults.length > 0 && (
          <div className="absolute bg-white w-full mt-32 shadow-lg z-10">
              ...
              <div
                ...
                onClick={() => {
                  setContacts((prev) => {
                    ...
                    if (index !== -1) {
                      // If already present, remove and add to the top.
                      const [found] = newContacts.splice(index, 1);
                      newContacts.unshift(found);
                    } else {
                      // Otherwise, add new contact to the top.
                      newContacts.unshift({
                        id: contact._id,
                        name: contact.name,
                        email: contact.email,
    ...
    {unreadCount > 0 && (
      <div
        ...
        className="fixed bottom-4 right-4 bg-green-500 text-white px-3 py-1 rounded-full cursor-pointer shadow-lg"
      >
        {unreadCount} New Message{unreadCount > 0 ? 's' : ''}
      </div>
    )}
      {/* Chat Panel */}
      <div className={showContactDetails ? "w-[60%] flex flex-col" : "flex-1 flex flex-col"}>
        ...
        {activeContact && (
          <div className="p-4 bg-gray-50 border-b border-gray-300 flex items-center">
            {/* Wrap profile icon and name in a clickable container */}
            <div
              ...
              >
                ...
                >
                  <path
                    ...
                    fill="#0869D9"
                  /* Remove or adjust the transform if needed:
      ...
      {/* Contact Details Panel (20% width) */}
